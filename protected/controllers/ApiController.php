...
class ApiController extends CController
{
     ...
     */
    Const APPLICATION_ID = 'INTERNATIONS';
    ...
    public function actionView()
    {
        ...
        switch($_GET['model'])
        {
            ...
            case 'users':
                ...
                break;
            case 'groups':
                $model = Groups::model()->findByPk($_GET['id']);
                break;
            default:
    ...
    public function actionCreate()
    {
        switch($_GET['model'])
        {
            ...
            case 'users_groups':
                ...
                $user_model = Users::model()->findByPk($_POST['user_id']);
                if(is_null($user_model))
                    return $this->_sendResponse(400, sprintf("Error: Didn't find any model <b>%s</b> with ID <b>%s</b>.",'user', $_POST['user_id']) );   
                $group_model = Groups::model()->findByPk($_POST['group_id']);
                if(is_null($group_model))
                    return $this->_sendResponse(400, sprintf("Error: Didn't find any model <b>%s</b> with ID <b>%s</b>.",'group', $_POST['group_id']) ); 
                break;
            default:
        ...
        foreach($_POST as $var=>$value) {
            ...
            if($model->hasAttribute($var) || $var == "password_confirmation") {
                $model->$var = $value;
            } else {
                // No, raise an error
    ...
    public function actionDelete()
    {
        switch($_GET['model'])
        {
            ...
            case 'groups':
                ...
                break;
            case 'users_groups':
                $model = UsersGroups::model()->findByPk($_GET['id']);          
                break;
            default:
